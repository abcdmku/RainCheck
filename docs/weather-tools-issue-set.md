...
2. Advanced weather prompts are mostly collapsed into `research-brief`. The schema knows about `hydrology`, `global-model`, and `radar-analysis`, but `classifyRequest()` does not emit those intents today.
...
| `get_short_range_model_guidance` | P0 | `/api/chat` lazy tool family `short-range-model` | New `shortRangeGuidanceSchema`; new `apps/api/src/weather/models.ts`; source catalog already has HRRR/GFS/GEFS scaffolds | `wx:model-short:v1:<model>:<cycle>:<lat>,<lon>:<window>` | `single-model-panel`, `meteogram` | Missing |
| `get_blend_and_analysis_guidance` | P0 | `/api/chat` lazy tool family `blend-analysis` | New `blendAnalysisGuidanceSchema`; new `apps/api/src/weather/models.ts`; add NBM/RTMA/URMA catalog entries | `wx:blend-analysis:v1:<product>:<cycle>:<lat>,<lon>:<window>` | `single-model-panel` or surface-analysis panel | Missing |
| `get_global_model_guidance` | P0 | `/api/chat` lazy tool family `global-model` | New `globalModelGuidanceSchema`; new `apps/api/src/weather/models.ts`; extend existing GFS/GEFS/ECMWF catalog entries | `wx:model-global:v1:<model>:<cycle>:<lat>,<lon>:<window>` | `single-model-panel` | Partial catalog scaffold only |
| `single-model-panel` artifact | P0 | Generated by live model-guidance tools when a preview panel is useful | `apps/api/src/weather/models.ts`, `apps/api/src/weather/service-client.ts` | `wx:model-compare:v1:<input-hash>` | `single-model-panel` | Implemented as artifact output, not a standalone comparison tool |
...
- Artifact contract:
  `single-model-panel`, optional `meteogram`
...
- Artifact contract:
  surface-analysis or single-model panel

### WX-P0-09 `get_global_model_guidance` and single-product analyst artifacts

- Scope:
  support GFS, GEFS, ECMWF IFS, and AIFS for days 2-10 questions
  require at least two model families before generating any single-product analyst artifact
